<template>
  <div class="pagination">
    ...
    <template v-for="(page,index) in startNumAndEndNum.end" >
        <button :key="index" v-if="page >= startNumAndEndNum.start"  @click="$emit('getPageNo',page)" :class="{active:page == pageNo}">{{page}}</button>
    </template>
...
export default {
  ...
  computed:{
    ...
    // 计算出连续页码的起始值{连续几页由continues决定}
    startNumAndEndNum(){
        let {continues,pageNo,totalPage} = this //结构过后下面的就不用写this.continues这种类型
        let start = 0 , end = 0
        // 总页数小于连续页数
